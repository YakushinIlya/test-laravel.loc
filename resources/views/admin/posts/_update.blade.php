...
@section('content')
    <div class="app-card app-card-orders-table shadow-sm mb-5 p-4">
        <div class="app-card-body">
            <form class="settings-form" method="post" action="{{route('admin.posts.update', ['id'=>$post['id']])}}" enctype="multipart/form-data">
                @method('PUT')
                @csrf
                <div class="mb-3">
                    <label for="setting-input-1" class="form-label">Заголовок</label>
                    <input type="text" name="head" class="form-control" id="setting-input-1" value="{{$post['head']}}" required>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <select name="category" class="form-control" id="setting-input-7" required>
                        ...
                        @foreach($categories as $category)
                            <option value="{{$category['id']}}" @if($category['id']==$post['category_id']) selected @endif>{{$category['head']}}</option>
                        @endforeach
                    </select>
                </div>
